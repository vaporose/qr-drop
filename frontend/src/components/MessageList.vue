...
<template>
    <div class="w-full max-w-xl rounded-md p-4 mb-4 h-64 overflow-y-auto chatbox">
      <div v-for="(msg, index) in props.messages" :key="index" class="mb-2">
        <div class="message-text px-3 py-2 rounded-md"
             :class="`message-${msg.sender_type}`">
          {{ msg.content }}
        </div>
        <div v-if="msg.sender_type !== 'system'" class="message-meta">
          <span class="message-identity">{{ msg.identity?.display_name }}</span>
